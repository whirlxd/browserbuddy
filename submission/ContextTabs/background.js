    ...
    extensionAPI.storage.local.get(null, (items) => {
        ...
        for (let contextName in predefinedContexts) {
            if (!existingKeys.includes(contextName)) {
                extensionAPI.storage.local.set({ [contextName]: predefinedContexts[contextName] }, () => {
